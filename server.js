...
let gameState = {
    locations: {
        bakhmut: { name: 'Бахмут', x: 390, y: 255, control: 'contested', rusPower: 45, ukrPower: 55 },
        avdeevka: { name: 'Авдеевка', x: 365, y: 280, control: 'contested', rusPower: 60, ukrPower: 40 },
        kherson: { name: 'Херсон', x: 240, y: 335, control: 'ukraine', rusPower: 8, ukrPower: 92 },
        donetsk: { name: 'Донецк', x: 430, y: 305, control: 'russia', rusPower: 95, ukrPower: 5 },
        lugansk: { name: 'Луганск', x: 490, y: 215, control: 'russia', rusPower: 92, ukrPower: 8 },
        zaporozhye: { name: 'Запорожье', x: 285, y: 285, control: 'contested', rusPower: 40, ukrPower: 60 },
        kharkiv: { name: 'Харьков', x: 435, y: 170, control: 'ukraine', rusPower: 10, ukrPower: 90 },
        mariupol: { name: 'Мариуполь', x: 445, y: 350, control: 'russia', rusPower: 85, ukrPower: 15 },
        soledar: { name: 'Соледар', x: 410, y: 235, control: 'russia', rusPower: 75, ukrPower: 25 },
        ugledar: { name: 'Угледар', x: 405, y: 320, control: 'contested', rusPower: 50, ukrPower: 50 }
    },
    players: {},
    squads: {},
    orders: {}
...
app.post('/api/move', (req, res) => {
    ...
    if (!player || !player.alive) {
        return res.json({ success: false, error: 'Нельзя переместиться' });
    }
    
    if (!gameState.locations[location]) {
        return res.json({ success: false, error: 'Точка не найдена' });
    }
    ...
    res.json({ 
        success: true, 
        player,
        message: `${player.name} переместился из ${oldLoc.name} в ${newLoc.name}`
    });
});
